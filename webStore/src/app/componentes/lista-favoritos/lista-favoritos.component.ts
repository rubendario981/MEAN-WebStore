...
export class ListaFavoritosComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.consultaBackEnd.validarFav(this.usuario._id).subscribe(
      res => {
        ...
        this.usuario.listaFavoritos.forEach(idProd => {
          this.consultaBackEnd.detalleProducto(idProd).subscribe(
            res => {
  ...
  eliminaFav(idProd) {
    ...
    this.consultaBackEnd.borrarFav(this.usuario).subscribe(
      res => {
        if (res) this.usuario = res.borraFav
        location.reload()
      },
      err => { console.log(err) }
    )
  ...
  borrarFavs(){    
    swal({
      ...
      if (willDelete) {
        this.consultaBackEnd.borrarFavlistado(this.usuario._id).subscribe(
          res => {
